...
fn print_help() {
    ...
    println!("  --allow-private-address  Permit non-global IP literals for controlled lab testing only; output is not valid public-node evidence");
    ...
    println!();
    println!("Example:");
    println!("  konofix-node.exe --port 45555 --public-host <PUBLIC_IP_OR_DNS> --status-interval 60 --health-file C:\\Konofix\\health.json --identity-file C:\\Konofix\\node-identity.key");
}
...
fn validate_public_host(
    ...
    match host.parse::<IpAddr>() {
        ...
        Ok(ip) => Err(format!(
            "Public host '{ip}' is private, local, CGNAT, documentation, benchmarking, multicast, reserved, or otherwise non-global under the Konofix evidence policy. Use --allow-private-address only for controlled lab testing."
        )),
        Err(_) => Ok(PublicHostPolicy::DnsUnverified),
    }
}

fn print_shareable_addresses(host: &str, port: u16, peer: PeerId, policy: PublicHostPolicy) {
    ...
    println!();
    match policy {
        PublicHostPolicy::GlobalIp => println!("=== KONOFIX SHAREABLE ADDRESSES ==="),
        PublicHostPolicy::DnsUnverified => {
            println!("=== KONOFIX CONFIGURED ADDRESSES (DNS UNVERIFIED) ===")
        }
        PublicHostPolicy::LabOnlyIp => println!("=== KONOFIX LAB-ONLY ADDRESSES ==="),
    }
    ...
    match policy {
        ...
        PublicHostPolicy::DnsUnverified => println!(
            "DNS UNVERIFIED: validate DNS resolution and external reachability with the public-node preflight/evidence tooling before treating this as public-node evidence."
        ),
        PublicHostPolicy::LabOnlyIp => println!(
            "LAB ONLY: this non-global IP literal was explicitly allowed and is NOT VALID PUBLIC-NODE EVIDENCE."
        ),
    }
    println!();
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(host) = &args.public_host {
        print_shareable_addresses(
            host,
            port,
            local_peer,
            public_host_policy.expect("public host policy exists when public host is present"),
        );
    } else {
        println!();
...
mod tests {
    ...
    fn public_host_policy_fails_closed_without_lab_override() {
        let error = validate_public_host("100.64.0.1", false)
            .expect_err("CGNAT literal must fail closed by default");
        assert!(error.contains("--allow-private-address"));

        let error = validate_public_host("203.0.113.10", false)
            .expect_err("documentation literal must fail closed by default");
        assert!(error.contains("non-global"));
    }

    #[test]
    fn public_host_policy_labels_lab_and_dns_modes() {
        assert_eq!(
            validate_public_host("10.0.0.5", true).expect("explicit lab override should pass"),
            PublicHostPolicy::LabOnlyIp
        );
        assert_eq!(
            validate_public_host("bootstrap.example.invalid", false)
                .expect("raw binary keeps DNS syntax unverified"),
            PublicHostPolicy::DnsUnverified
        );
        assert_eq!(
            validate_public_host("1.1.1.1", false).expect("global IPv4 should pass"),
            PublicHostPolicy::GlobalIp
        );
    }
