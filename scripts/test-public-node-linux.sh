...
if grep -Fq -- '--allow-private-address' <<<"$unit"; then
  fail 'Normal public-node unit must not enable the lab-only private-address override.'
fi
...
grep -Fq -- '--allow-private-address' <<<"$lab_unit" || fail 'Lab systemd unit must propagate the explicit binary lab override.'
