...
Regression tests cover pre-existing partial files with sentinel bytes, concurrent reservations for the same requested filename, a final destination appearing after temporary reservation, bounded reservation exhaustion, no-clobber finalization, successful promotion, multi-byte filename byte bounds plus retry/temporary suffix geometry, encoded-byte file-offer admission boundaries, request-codec chunk headroom, the 16 KiB response ceiling, path-free completion acknowledgements, pending-offer TTL boundaries, accepted-transfer inactivity TTL boundaries, final-connection-only cleanup, peer-scoped outgoing reclamation and scoped request-metadata pruning. Cancellation, rejected/failed acceptance, hash mismatch, disconnect, inactivity timeout and finalization failure clean up only state owned by the affected transfer.
