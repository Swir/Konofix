...
All testers should use the same package and keep its ZIP, checksum, `BUILD_INFO.json`, tools and evidence together. Do not mix this build with historical `test1` or the withdrawn `5e8b20b2` candidate. See `TESTER_HANDOFF.md`, `TESTING.md` and `GLOBAL_BETA.md` in the ZIP. The optional `validate-global-beta-evidence.mjs` evidence tool requires Node.js 22+ on the evidence collector's computer; the desktop chat does not require Node.js.
