...
- added campaign IDs to generated schema-v3 network reports and added `scripts/new-network-test-campaign.ps1` to create TCP/QUIC/Relay/DCUtR/CGNAT PENDING reports as one coherent test campaign,
- hardened stable promotion so the required manifests must share exactly one canonical campaign ID plus the same oriented Client A/Client B, country and network/operator metadata; mixed sessions and swapped client roles are rejected,
- added adversarial campaign CI tests for missing/malformed/mixed campaign IDs, mixed clients, swapped endpoints and changed country/network metadata,
- preserved campaign identity through the report editor, exact-build promotion preflight and human-readable Markdown output,
- bundled the campaign generator into verified Windows artifacts, added its own fail-closed self-tests, and mirrored that test path in local preflight,
- added `scripts/check-promotion-evidence.ps1`, a bundled exact-build preflight that requires verified `BUILD_INFO.json`, the complete schema-v3 TCP/QUIC/Relay/DCUtR/CGNAT evidence set and matching public-Node soak history to agree on version, source commit, campaign identity and bootstrap Peer ID before returning PASS,
...
- aligned `scripts/check.ps1` with the CI preflight path so local checks run release/network/campaign/bootstrap/public-Node/readiness/health/soak/promotion gates, project audit, deterministic `npm ci`, frontend build, locked Rust metadata/tests and application/Node checks,
...
- strengthened release verification so provenance, committed frontend/Rust lockfiles, Node-soak instructions, campaign tooling, test-tool hashes and all installer hashes are cross-checked after ZIP extraction before artifact upload,
