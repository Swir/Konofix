...
if ($version -cne '0.4.2') { throw 'This beta intent belongs to version 0.4.2.' }
$tag = 'v0.4.2-beta.1'
...
try {
    ...
    $installerName = 'Konofix-Chat-0.4.2-beta.1-setup.exe'
    $installerPath = Join-Path $output.FullName $installerName
    $source = 'bundle/nsis/Konofix Chat_0.4.2_x64-setup.exe'
...
$notes = Get-Content 'docs/RELEASE_0.4.2_BETA1.md' -Raw
