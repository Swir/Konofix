...
- Internet bootstrap precheck now strictly validates TCP and QUIC-v1 multiaddr structure, address families, ports and Peer IDs, with adversarial CI self-tests before any real-network evidence is accepted.
- Windows test archives now carry the operational bootstrap/evidence/health/soak scripts themselves, and artifact provenance verifies their hashes and sizes so remote testers can run the documented flow without cloning the source repository.
