...
if grep -Fq -- '--allow-private-address' <<<"$unit"; then
  fail 'Normal public systemd unit must not enable the lab-only private-address override.'
fi
...
grep -Fq -- '--allow-private-address' <<<"$lab_unit" || fail 'Lab override must be forwarded to the raw Node in the systemd unit.'
