...
if ($version -cne '0.5.1') { throw 'This beta intent belongs to version 0.5.1.' }
$tag = 'v0.5.1-beta.1'
...
try {
    ...
    $installerName = 'Konofix-Chat-0.5.1-beta.1-setup.exe'
    $installerPath = Join-Path $output.FullName $installerName
    $source = 'bundle/nsis/Konofix Chat_0.5.1_x64-setup.exe'
...
$notes = Get-Content 'docs/RELEASE_0.5.1_BETA1.md' -Raw
