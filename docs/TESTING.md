...
Windows CI also installs the generated NSIS package into its disposable runner account, checks that the Start menu shortcut targets the exact `konofix-chat.exe`, and verifies that the bundled login page and event listen/unlisten IPC work in WebView2. It resolves the MSI File table to CAB identifiers, compares both installer payloads against the production executable with Tauri's documented per-format bundle marker, and checks the Windows GUI subsystem. No other binary differences are accepted. This catches accidental packaging of the Node or Netprobe helper as the desktop app. `scripts/test-chat-installer.ps1` is restricted to disposable CI runners because it installs and uninstalls the app. The test uses temporary app-specific WebView2 machine policies for a loopback debug port and isolated profile because elevated hosts ignore environment overrides in WebView2 150+. It refuses existing matching policies and removes only the values it created. The shipped app never enables this debug port. Successful pull-request runs upload an exact-build test bundle; its merge-ref commit is recorded in `BUILD_INFO.json` and it remains a test artifact, not a published release. Failed startup runs retain clearly named diagnostic binaries for three days; those unqualified binaries must not be published as a release.
