...
Regression tests cover pre-existing partial files with sentinel bytes, concurrent reservations for the same requested filename, a final destination appearing after temporary reservation, bounded reservation exhaustion, no-clobber finalization, successful promotion, multi-byte filename byte bounds plus retry/temporary suffix geometry, pending-offer TTL boundaries, accepted-transfer inactivity TTL boundaries, final-connection-only cleanup, peer-scoped outgoing reclamation, explicit-cancel request-metadata pruning and transfer-specific pending-offer UI invalidation. Cancellation, rejected/failed acceptance, hash mismatch, disconnect, inactivity timeout and finalization failure clean up only state owned by the affected transfer. Adversarial policy mutations cover local/remote metadata pruning, peer ownership, sender-cancelled UI routing, the independent TTL-expiry listener and explicit localization in every supported dictionary.
