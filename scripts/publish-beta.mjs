...
export function assertPlan(plan, env, readFile) {
  ...
  assert.equal(plan.version, '0.5.1');
  ...
  const archive = `Konofix-Chat-0.5.1-Windows-${plan.commit}.zip`;
  const expected = ['BUILD_INFO.json', 'Konofix-Chat-0.5.1-beta.1-setup.exe', 'Konofix-Chat-0.5.1-beta.1-setup.exe.sha256', archive, `${archive}.sha256`].sort();
...
export async function publishBeta(plan, api, readFile) {
  ...
  if (release) {
    ...
    release = await api('POST', '/releases', { tag_name: tag, target_commitish: plan.commit, name: 'Konofix Chat 0.5.1 Beta 1 — Transfer & Private Attention Preview', body: plan.body, draft: true, prerelease: true, make_latest: 'false' });
  }
