...
The percentage is calculated from the checked tasks in the active `0.4.2 — Real Internet Test` roadmap milestone (54 of 59 tasks complete). It is intentionally not increased by CI runs alone: the remaining public-Node, cross-country, CGNAT, transport and real-world-fix work must actually pass before the milestone can reach 100%.
...
The installer stages `konofix-node.exe` and the validated launcher under persistent state, runs it as SYSTEM at boot, configures restart-on-failure, and can create only the scoped inbound TCP/UDP rules for the selected port. `-Uninstall` removes the task and its firewall group but deliberately preserves Node identity/state, preventing accidental bootstrap Peer-ID rotation.
...
Record observations with the bundled report editor. PASS and FAIL results require a non-empty evidence note, and a file-transfer PASS must include the actual full SHA-256 digest:

```powershell
.\scripts\set-network-test-result.ps1 `
  -Manifest .\test-results\konofix-real-network-...\network-test-tcp-....json `
  -Check file_a_to_b_sha256 `
  -Result PASS `
  -Evidence "sender/receiver sha256=0123456789abcdef...64_hex_characters_total..."
```
...
It returns PASS only when all required scenarios are valid and fresh, all five belong to the same exact-build session endpoint pair and paired TCP/QUIC bootstrap identity, every PASS carries concrete evidence (including real file digests), and the soak history matches the exact artifact version, source commit and single bootstrap Peer ID.
