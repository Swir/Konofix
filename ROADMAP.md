...
The active milestone currently has 44 of 49 tasks complete. The remaining five tasks require real public-network evidence and fixes discovered during those tests; CI alone cannot credit them.
...
- [x] harden Node state-file writes so the raw binary rejects identity/health/executable collisions, uses unique create-new health temp files, and fails startup when an explicitly requested initial health snapshot cannot be published
...
- Local preflight mirrors the CI gate/test path instead of checking only the frontend and basic Cargo compilation, including deterministic `npm ci` against the committed frontend lockfile.
...
- Windows test archives carry the operational public-Node/bootstrap/evidence/health/soak scripts themselves, and artifact provenance verifies their hashes and sizes so remote testers can run the documented flow without cloning the source repository.
- Windows artifacts carry machine-readable `BUILD_INFO.json` provenance with commit/version plus hashes and sizes for the Node, committed frontend lockfile, captured Rust dependency-resolution record, test tools and every installer; artifact verification cross-checks that metadata before upload.
...
The existing `v0.4.2-test1` pre-release is a controlled preview for gathering real-network evidence, not proof that the public-network milestone has passed. No later release should be published automatically from an ordinary push. A new release may be published only when Windows CI is green, production application and Node bundles exist, version/project audits pass, the ZIP and SHA-256 pass verification, instructions are included, and there is a real publicly reachable bootstrap path. Pull requests to `main` must already reproduce the production application/Node build and pass staged archive verification before merge; artifact upload remains a post-merge `main` action. Promotion beyond the test release additionally requires fresh schema-v3 machine-readable PASS evidence for the required real-network scenarios from the exact release client and Node version and exact source commit, with independent countries and networks/operators recorded and one stable public bootstrap Peer ID across the promotion evidence. Stable promotion also requires a continuous Node soak history; the release gate binds that history to the same bootstrap Peer ID, Node version and exact source commit used by the network manifests and rejects restarts, stale samples and monitoring gaps. Public/community Node operators can pin identity storage explicitly, malformed existing identities fail startup rather than silently rotating the Peer ID, and the bundled deployment preflight rejects obviously non-public addresses and unsafe state-path collisions before launch. The Node binary independently rejects identity/health/executable path collisions and will not continue startup if an explicitly requested initial health snapshot cannot be safely published. Windows CI embeds `BUILD_INFO.json` in each test archive and verifies that its commit/version plus Node/installer/test-tool/frontend-lock hashes and sizes match the packaged files before upload. The Node embeds the build source commit into schema-v2 health snapshots, while real-network manifests use schema v3, preventing evidence gathered from one `0.4.2` commit from promoting a different `0.4.2` build. Frontend dependency resolution is deterministic: the committed `package-lock.json` is validated against `package.json`, Windows CI and local preflight install with `npm ci`, setup-node caching is keyed to the lockfile, and release verification requires the packaged lockfile to match the committed build input. The captured `Cargo.lock` records the Rust graph resolved for that build but is not yet treated as a reproducibility guarantee until it is committed and enforced as a build input. GitHub Actions are pinned to full commit SHAs and checkout credentials are not persisted.
