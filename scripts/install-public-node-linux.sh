...
if ((ALLOW_PRIVATE)); then
  NODE_LAB_ARG=" --allow-private-address"
fi
...
User=${SERVICE_USER}
Group=${SERVICE_USER}
ExecStart=${INSTALLED_BINARY} --port ${PORT} --public-host ${PUBLIC_HOST} --status-interval ${STATUS_INTERVAL} --health-file ${HEALTH_FILE} --identity-file ${IDENTITY_FILE}${NODE_LAB_ARG}
Restart=on-failure
RestartSec=5s
TimeoutStopSec=30s
UMask=0077
