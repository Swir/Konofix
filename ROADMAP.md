...
The active milestone currently has 39 of 44 tasks complete. The remaining five tasks require real public-network evidence and fixes discovered during those tests; CI alone cannot credit them.
...
- Windows artifacts carry machine-readable `BUILD_INFO.json` provenance with commit/version plus hashes and sizes for the Node, test tools and every installer; artifact verification cross-checks that metadata and the captured Rust dependency-resolution record before upload.
